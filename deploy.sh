...
DOMAIN_NAME="yourgamingtoolkit.com"
EMAIL="hello@joshpayette.dev"

# Script Vars
REPO_URL="https://github.com/joshpayette/your-gaming-toolkit.git"
APP_DIR=~/ygt
...
DATABASE_URL="postgres://$MYSQL_USER:$MYSQL_PASSWORD@db:5432/$MYSQL_DB"

# For external tools (like Drizzle Studio)
DATABASE_URL_EXTERNAL="postgres://$MYSQL_USER:$MYSQL_PASSWORD@localhost:5432/$MYSQL_DB"

# Create the .env file inside the app directory (~/ygt/.env)
...
sudo rm -f /etc/nginx/sites-available/ygt
sudo rm -f /etc/nginx/sites-enabled/ygt
...
sudo cat > /etc/nginx/sites-available/ygt <<EOL
...
sudo ln -s /etc/nginx/sites-available/ygt /etc/nginx/sites-enabled/ygt

# Restart Nginx to apply the new configuration
sudo systemctl restart nginx

# Build and run the Docker containers from the app directory (~/ygt)
cd $APP_DIR
